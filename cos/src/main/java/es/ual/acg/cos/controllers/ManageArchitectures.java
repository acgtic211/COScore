...
public class ManageArchitectures {
	...
	private void initializeDataStore() {
		...
		//hibernateProperties.setProperty(PersistenceOptions.INHERITANCE_MAPPING, InheritanceType.SINGLE_TABLE.name());
		...
		//hibernateProperties.setProperty("teneo.mapping.fetch_one_to_many_extra_lazy", "true");
		//hibernateProperties.setProperty("teneo.runtime.handle_unset_as_null", "true");
	...
	public String exportCAMFromString(String camFileType, String camFileString)
	{
		...
		if(cam != null) {
			...
			Query query  = session.createQuery("FROM ConcreteArchitecturalModel cam WHERE cam.camID = '" +
					camID + "'");
...
//		}
		...
		if(aam != null)
		{
			...
			//Check if the CAM ID does exist
			Query query  = session.createQuery("FROM AbstractArchitecturalModel aam WHERE aam.aamID = '" +
					aamID + "'");
			List<?> ccsList = query.list();
			if(ccsList.size()==0)
			{
...
//	    }
	...
	public String addComponent(String camID, ConcreteComponent concreteComponent, Port inputPort,
			List<RuntimeProperty> runtimePropertyList) throws Exception{	
		SessionFactory sessionFactory = dataStore.getSessionFactory();
	...
	public void saveComponentInstance(String componentName, String componentAlias, String componentInstance) throws Exception{	
		...
		try{
			...
			Query query = session.createQuery("FROM ConcreteComponent WHERE componentname = '"+ componentName + "' AND componentalias= '"+ componentAlias +"'");
	...
	public void changeComponentProperties(String componentInstance,String componentNewInstance, String componentNewName,String componentNewAlias)throws Exception{
		SessionFactory sessionFactory = dataStore.getSessionFactory();
	...
	public void changeComponentService(String componentInstance, int numberService, List<RuntimeProperty> listServices) throws Exception{
		SessionFactory sessionFactory = dataStore.getSessionFactory();
